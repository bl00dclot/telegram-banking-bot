...
/**
 * Dispatches incoming Telegram updates to either: - an active conversation
 * handler (if user is mid-conversation), or - a registered BotCommand (if
 * message is a /command).
 * <p>
 * Constructor requires a ConversationManager and ConversationRouter so
 * follow-up messages (e.g. password input) are routed before usual command
 * parsing.
 */
public class CommandDispatcher {
	...
	private final Set<String> safeCmds;

	public CommandDispatcher(Set<Long> adminList, ConversationManager convMgr, ConversationRouter convRouter,
			SessionManager sessionManager, Set<String> safeCmds) throws Exception {
		this.adminList = adminList;
		System.out.println(adminList);
		this.convRouter = convRouter;
		this.convMgr = convMgr;
		this.sessionManager = sessionManager;
		this.safeCmds = safeCmds;
	}

	/**
	 * Dispatch an Update. Returns true if the update was handled (either routed to
	 * a conversation or a command). Returns false if it was not a command (so
	 * higher-level code can handle non-command updates).
	 */
			...
			if (text.startsWith("/")) {
				...
				if (cmd != null && safeCmds.contains(token)) {
					...
					try {
						...
						e.printStackTrace();
						ctx.bot.sendText(msg.getChatId(), "Internal error.");
					}
					return true;
				}
	...
	/**
	 * Register a command instance (token from cmd.name() will be normalized to
	 * lower-case)
	 */
