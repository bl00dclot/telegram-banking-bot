...
public class UserDaoImpl implements UserDao {
	...
	@Override
	public boolean updateBalances(Connection conn, int userId, double deltaGold, double deltaRealUsd,
			double deltaExpectedUsd) throws SQLException {
	...
	public boolean updateGoldBalance(Connection conn, int userId, int deltaGold) throws SQLException {
		String sql = "UPDATE users SET gold_balance = gold_balance + ? " + "WHERE id = ? AND gold_balance + ? >= 0";
		try (PreparedStatement ps = conn.prepareStatement(sql)) {
	...
	public boolean updateExpectedUsdBalance(Connection conn, int userId, int deltaExpectedUsd) throws SQLException {
		String sql = "UPDATE users SET expected_usd_total = expected_usd_total + ? " + "WHERE id = ?";
		try (PreparedStatement ps = conn.prepareStatement(sql)) {
	...
	@Override
	public boolean isAuthorized(Connection conn, long telegramId) throws SQLException {
		String sql = "SELECT authorized FROM users WHERE telegram_id = ?";
		try(PreparedStatement ps = conn.prepareStatement(sql)){
			ps.setLong(1, telegramId);
			ResultSet rs = ps.executeQuery();
			return rs.next();
